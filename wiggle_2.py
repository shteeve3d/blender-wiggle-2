...
def reset_bone(b):
    b.wiggle.position = b.wiggle.position_last = (
        b.id_data.matrix_world @ Matrix.Translation(b.tail)
    ).translation
    b.wiggle.position_head = b.wiggle.position_last_head = (
...
def get_parent(b):
    ...
    par = (
        ...
        if (
            p.wiggle_enable
            and (not p.wiggle_mute)
            and ((p.wiggle_head and not p.bone.use_connect) or p.wiggle_tail)
        )
        else get_parent(p)
    )
    return par
...
def collide(b, dg, head=False):
    ...
    for collider in colliders:
        ...
        if (
            (n.dot(v.normalized()) > 0.01)
            or (v.length < radius)
            or (co and (v.length < (radius + sticky)))
        ):
...
def update_matrix(b, last=False):
    ...
    if p:
        ...
        if b.bone.inherit_scale == "FULL":
            ...
            ro = (
                p.wiggle.matrix.to_quaternion().to_matrix().to_4x4()
                @ diff.to_quaternion().to_matrix().to_4x4()
            )
    ...
    if b.wiggle_head and not b.bone.use_connect:
        ...
        loc = Matrix.Translation(
            relative_matrix(mat, Matrix.Translation(b.wiggle.position_head)).translation
        )
        mat = m2
    ...
    if b.bone.inherit_scale == "FULL":
        ...
        if par:
            sy = (
                b.id_data.matrix_world
                @ par.matrix
                @ relative_matrix(par.matrix, b.matrix).translation
                - b.wiggle.position
            ).length / length_world(b)
            if p:
                sy = (
                    p.wiggle.matrix @ relative_matrix(p.matrix, b.matrix).translation
                    - b.wiggle.position
                ).length / length_world(b)
        else:
            sy = (
                b.id_data.matrix_world @ b.matrix.translation - b.wiggle.position
            ).length / length_world(b)
    ...
    if last:
        ...
        for c in b.constraints:
            if c.enabled and not (
                ...
                const = True
        if const:
            print(b.name, " HAS CONTRAINT", [c.type for c in b.constraints if c.enabled])
...
def move(b, dg):
    ...
    if dt:
        if b.wiggle_tail:
            ...
            if b.wiggle_wind_ob:
                dir = b.wiggle_wind_ob.matrix_world.to_quaternion().to_matrix().to_4x4() @ Vector(
                    (0, 0, 1)
                )
                fac = 1 - b.wiggle_wind_ob.field.wind_factor * abs(
                    dir.dot((b.wiggle.position - b.wiggle.matrix.translation).normalized())
                )
        ...
        if b.wiggle_head and not b.bone.use_connect:
            ...
            if b.wiggle_wind_ob_head:
                dir = (
                    b.wiggle_wind_ob_head.matrix_world.to_quaternion().to_matrix().to_4x4()
                    @ Vector((0, 0, 1))
                )
                F += (
                    dir
                    * b.wiggle_wind_ob_head.field.strength
                    * b.wiggle_wind_head
                    / b.wiggle_mass_head
                )
    ...
    if dt:
        ...
        if b.wiggle_head and not b.bone.use_connect:
            ...
            mat = Matrix.LocRotScale(
                b.wiggle.position_head, mat.decompose()[1], b.matrix.decompose()[2]
            )
            ...
            mat = Matrix.LocRotScale(
                mat.decompose()[0], mat.decompose()[1], b.matrix.decompose()[2]
            )
            target = mat @ Vector((0, b.bone.length, 0))
            s = spring(target, b.wiggle.position, b.wiggle_stiff)
        ...
        if b.wiggle_head and not b.bone.use_connect:
            if p:
                if b.parent == p and p.wiggle_tail:
                    target = (
                        p.wiggle.position
                        + (b.wiggle.position_head - p.wiggle.position).normalized()
                        * (b.id_data.matrix_world @ b.head - b.id_data.matrix_world @ p.tail).length
                    )
                else:  # indirect
                    targetpos = (
                        p.wiggle.matrix
                        @ relative_matrix(p.matrix, b.parent.matrix)
                        @ Vector((0, b.parent.length, 0))
                    )
                    target = (
                        targetpos
                        + (b.wiggle.position_head - targetpos).normalized()
                        * (
                            b.id_data.matrix_world @ b.head - b.id_data.matrix_world @ b.parent.tail
                        ).length
                    )
            elif b.parent:
                ...
                target = (
                    ptail
                    + (b.wiggle.position_head - ptail).normalized()
                    * (
                        b.id_data.matrix_world @ b.head - b.id_data.matrix_world @ b.parent.tail
                    ).length
                )
            ...
            if p and b.wiggle_chain_head:
                if p.wiggle_tail:
                    ...
                    ratio = (p.wiggle.matrix.translation - p.wiggle.position).length / (
                        p.wiggle.matrix.translation - tailpos
                    ).length
                    tailpos -= s * fac
                    p.wiggle.position -= s * ratio * fac
                else:  # DOES THIS ASSUME ANYTHING? (No, head only translates, no bone stretching)
                    fac = (
                        get_fac(b.wiggle_mass_head, p.wiggle_mass_head)
                        if i
                        else p.wiggle_stretch_head
                    )
                    p.wiggle.position_head -= s * fac
                b.wiggle.position_head += s * (1 - fac)
            else:
            ...
            target = b.wiggle.position_head + (
                b.wiggle.position - b.wiggle.position_head
            ).normalized() * length_world(b)
            ...
            target = mat.translation + (
                b.wiggle.position - mat.translation
            ).normalized() * length_world(b)
            s = stretch(target, b.wiggle.position, b.wiggle_stretch)
...
def wiggle_pre(scene):
    if (scene.wiggle.lastframe == scene.frame_current) and not scene.wiggle.reset:
        return
    if scene.wiggle.is_rendering:
        return
        ...
        for wb in wo.list:
            ...
            if not b.wiggle.collision_col:
                if b.wiggle_collider_collection:
                    b.wiggle_collider_collection = bpy.data.collections.get(
                        b.wiggle_collider_collection.name
                    )
...
def wiggle_post(scene, dg):
    ...
    if (
        ...
        and (scene.wiggle.loop == False)
        and (scene.wiggle.is_preroll == False)
    ):
    ...
    if frames_elapsed > 4:
        frames_elapsed = 1  # handle large jumps?
    if scene.wiggle.is_preroll:
        frames_elapsed = 1
        ...
        if frames_elapsed:
            for b in bones:
                ...
                if b.wiggle.collision_normal.length:
                    vb = (
                        b.wiggle.velocity.reflect(b.wiggle.collision_normal).project(
                            b.wiggle.collision_normal
                        )
                        * b.wiggle_bounce
                    )
                b.wiggle.velocity = (b.wiggle.position - b.wiggle.position_last) / max(
                    frames_elapsed, 1
                ) + vb
                vb = Vector((0, 0, 0))
                if b.wiggle.collision_normal_head.length:
                    vb = (
                        b.wiggle.velocity_head.reflect(b.wiggle.collision_normal_head).project(
                            b.wiggle.collision_normal_head
                        )
                        * b.wiggle_bounce_head
                    )
                b.wiggle.velocity_head = (
                    b.wiggle.position_head - b.wiggle.position_last_head
                ) / max(frames_elapsed, 1) + vb
...
class WiggleBake(bpy.types.Operator):
    ...
    def execute(self, context):
        def push_nla():
            if context.scene.wiggle.bake_overwrite:
                return
            if not context.scene.wiggle.bake_nla:
                return
...
class WIGGLE_PT_Head(WigglePanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (
            context.scene.wiggle_enable
            and context.object
            and not context.object.wiggle_mute
            and context.active_pose_bone
            and not context.active_pose_bone.wiggle_mute
            and not context.active_pose_bone.bone.use_connect
        )
    ...
    def draw(self, context):
        ...
        drawprops(
            col,
            b,
            ["wiggle_mass_head", "wiggle_stiff_head", "wiggle_stretch_head", "wiggle_damp_head"],
        )
        col.separator()
...
class WIGGLE_PT_Tail(WigglePanel, bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if collision:
            col = layout.column(align=True)
            drawprops(
                col, b, ["wiggle_radius", "wiggle_friction", "wiggle_bounce", "wiggle_sticky"]
            )
...
class WIGGLE_PT_Bake(WigglePanel, bpy.types.Panel):
    ...
    def poll(cls, context):
        return (
            context.scene.wiggle_enable and context.object.wiggle_enable and context.mode == "POSE"
        )
...
class WiggleItem(bpy.types.PropertyGroup):
    ...
    list: bpy.props.CollectionProperty(
        type=WiggleBoneItem, override={"LIBRARY_OVERRIDABLE", "USE_INSERTION"}
    )
...
class WiggleBone(bpy.types.PropertyGroup):
    matrix: bpy.props.FloatVectorProperty(
        name="Matrix", size=16, subtype="MATRIX", override={"LIBRARY_OVERRIDABLE"}
    )
    ...
    position_last: bpy.props.FloatVectorProperty(
        subtype="TRANSLATION", override={"LIBRARY_OVERRIDABLE"}
    )
    ...
    collision_col: bpy.props.PointerProperty(
        type=bpy.types.Collection, override={"LIBRARY_OVERRIDABLE"}
    )
    ...
    velocity_head: bpy.props.FloatVectorProperty(
        subtype="VELOCITY", override={"LIBRARY_OVERRIDABLE"}
    )
...
class WiggleScene(bpy.types.PropertyGroup):
    ...
    iterations: bpy.props.IntProperty(
        name="Quality",
        description="Constraint solver interations for chain physics",
        min=1,
        default=2,
        soft_max=10,
        max=100,
    ...
    loop: bpy.props.BoolProperty(
        name="Loop Physics", description="Physics continues as timeline loops", default=True
    )
    list: bpy.props.CollectionProperty(
        type=WiggleItem, override={"LIBRARY_OVERRIDABLE", "USE_INSERTION"}
    )
    preroll: bpy.props.IntProperty(
        name="Preroll", description="Frames to run simulation before bake", min=0, default=0
    )
    is_preroll: bpy.props.BoolProperty(default=False)
    bake_overwrite: bpy.props.BoolProperty(
        name="Overwrite Current Action",
        description="Bake wiggle into current action, instead of creating a new one",
        default=False,
    )
    bake_nla: bpy.props.BoolProperty(
        name="Current Action to NLA",
        description="Move existing animation on the armature into an NLA strip",
        default=False,
    )
...
def register():
    ...
    bpy.types.PoseBone.wiggle_collider_type = bpy.props.EnumProperty(
        name="Collider Type",
        items=[
            ("Object", "Object", "Collide with a selected mesh"),
            ("Collection", "Collection", "Collide with all meshes in selected collection"),
        ],
        override={"LIBRARY_OVERRIDABLE"},
